Nue.components.NueTomSelect = {
  dataAttributeName: 'data-nue-tom-select-options',
  defaults: {
  ...
  _init() {
    ...
    for (let i = 0; i < that.collection.length; i += 1) {
      ...
      if (_options.plugins.hasOwnProperty('nue_smart_position') && !_$el.closest('.modal')) {
      ...
      TomSelect.define('nue_smart_position', function(plugin_options) {
        function smartPositionOnScroll(instance) {
          ...
          if (menuBoundary.bottom > window.innerHeight) {
            ...
            instance.$menu.style.top = `-${(instance.$menu.clientHeight + 10)}px`
          } else if (menuBoundary.top < 0) {
            instance.$menu.style.top = null
          }
        }
  ...
  singleMultiple(tomSelect, settings) {
    tomSelect.control.classList.add('nue-select-single-multiple')
