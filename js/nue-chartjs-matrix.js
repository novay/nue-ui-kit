Nue.components.NueChartMatrixJS = {
  ...
  legend(settings) {
    if (settings.options.hasOwnProperty('matrixLegend')) {
      ...
      $legendContainer.classList.add('nue-chartjs-matrix-legend')

      $legendContainer.insertAdjacentHTML('beforeend', `<li class="nue-chartjs-matrix-legend-min">${min}</li>`)

      for (var i = 0; i < max;) {
        var value = i;
        var alpha = (additionToValue + value) / settings.options.matrixBackgroundColord.accent;
        html.push('<li class="nue-chartjs-matrix-legend-item" style="background-color: ' + Chart.helpers.color(settings.options.matrixBackgroundColord.color).alpha(alpha).rgbString() + '"></li>');
      ...
      $legendContainer.insertAdjacentHTML('beforeend', `<li class="nue-chartjs-matrix-legend-max">${max}</li>`)
    }
  },
  ...
  tooltip() {
    window.addEventListener('mousemove', function (e) {
      if (!e.target.closest('canvas')) {
        let $el = document.querySelector('.nue-chartjs-tooltip-matrix')
        if ($el) {
  ...
  extendOptions(el, options, dataSettings, index) {
    ...
    settings.options.plugins.tooltip.external = function (tooltipModel) {
      ...
      if (!tooltipEl) {
        ...
        tooltipEl.classList.add('nue-chartjs-tooltip-matrix');
