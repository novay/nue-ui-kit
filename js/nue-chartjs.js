Nue.components.NueChartJS = {
	dataAttributeName: 'data-nue-chartjs-options',
	defaults: {
	...
	init(el, options, id) {
		...
		window.addEventListener('on-nue-appearance-change', e => this._updateTheme(e.detail))
	...
	getTheme(index, theme) {
		if (!this.themes[index] || !this.themes[index][theme])  {
			console.error(`The element '${index}' or theme '${theme}' was not found in the NueChartjs theme list.`)
		}
	...
	extendChart(el, settings, newChartJS) {
		...
		if (settings.type === 'line' && settings.options.plugins.tooltip.lineMode) {
			...
			newChartJS.draw = function (ease) {
				...
				if (this.tooltip._active && this.tooltip._active.length) {
					let activePoint = this.tooltip._active[0],
						el = this.canvas,
						tooltipWrap = document.querySelector('.nue-chartjs-tooltip-wrap'),
					...
					let $nueTooltip = document.querySelector('.nue-chartjs-tooltip')

					if (tooltipWrap.offsetLeft + tooltipWrap.clientWidth > (el.offsetLeft + el.clientWidth) - 100) {
						if (!$nueTooltip) return
						$nueTooltip.classList.remove('nue-chartjs-tooltip-right')
						$nueTooltip.classList.add('nue-chartjs-tooltip-left')
					} else {
						if (!$nueTooltip) return
						$nueTooltip.classList.add('nue-chartjs-tooltip-right')
						$nueTooltip.classList.remove('nue-chartjs-tooltip-left')
					}
			...
			el.addEventListener('toucnuetart', () => {
			...
			el.addEventListener('mousemove', (evt) => {
				let $wrap = document.querySelector('.nue-chartjs-tooltip-wrap'),
					offset = getOffset(el)

				if (!$wrap) return
				if (evt.pageY - offset.top > $wrap.clientHeight / 2 && (evt.pageY - offset.top) + ($wrap.offsetHeight / 2) < el.clientHeight) {
					document.querySelector('.nue-chartjs-tooltip').style.top = `${((evt.pageY + $wrap.clientHeight / 2) - (offset.top + el.clientHeight / 2))}px`
				}
			})
		}
	},
					...
					tooltip: {
						external: function (tooltipModel) {
							...
							if (!tooltipEl) {
								...
								tooltipEl.classList.add('nue-chartjs-tooltip-wrap')
								tooltipEl.innerHTML = '<div class="nue-chartjs-tooltip"></div>'
							...
							if (tooltipModel.tooltip.body) {
								...
								var innerHtml = '<header class="nue-chartjs-tooltip-header">'

								titleLines.forEach(title => {
									innerHtml += this.options.yearStamp ? title + ', ' + today.getFullYear() : title
								})

								innerHtml += '</header><div class="nue-chartjs-tooltip-body">'
								...
								var tooltipRoot = tooltipEl.querySelector('.nue-chartjs-tooltip')
	...
	_setTheme (settings, index) {
		...
		let theme = localStorage.getItem('nue_theme') || window.nue_config.themeAppearance.layoutSkin
